<?php
    ...
    $query = "SELECT recent_viewed.*, car.*
                FROM recent_viewed
                JOIN car ON recent_viewed.car_id = car.id
                WHERE recent_viewed.user_id = {$_SESSION['user']['id']}
                ORDER BY recent_viewed.updated_at DESC";

    $result = mysqli_query($conn, $query);
    $recentlyViewedCars = mysqli_fetch_all($result, MYSQLI_ASSOC);
...
<body>
    ...
    <div class="container">
        <div class="row mt-4">
            ...
            <hr>
            <?php
                foreach ($recentlyViewedCars as $car) {
                    ?>
